...
function Resource(ret, host, opts) {
  //ret 代表所有文件file信息，host代表入口html的file，opts用户传入插件的参数
  this._ret = ret;
  this._host = host;
  this._opts = opts;
  this._map = ret.map;
...
Resource.prototype.getFileByUrl = function(url) {
  ...
  if (!file) {
    ...
    var info = fis.project.lookup(url, this._host, true);
    if (info.file) {
...
Resource.prototype.remove = function(id, deffer) {
  ...
  deffer || node.extras && node.extras.async && node.extras.async.forEach(function(res) {
...
Resource.prototype.buildConf = function(type) {
  ...
  }else if (/^asyncsMod/i.test(type)) {
    return this.buildAsyncsResourceMap();
...
Resource.prototype.buildResourceMap = function() {
  ...
  Object.keys(this.loaded).forEach(function(id) {
    ...
    var uri = node.uri;
    var message = {
      target: uri, // 目标文件对象，或者目标文件的绝对 url
      file: self._host // target 相对的文件。
    };
    ...
    if (node.pkg) {
      item.pkg = node.pkg;
      delete item.url; // 如果有 pkg，这个属性并没有用
...
Resource.prototype.buildAsyncsResourceMap = function() {
  var self = this;
  var res = {};
  var pkg = {};
  ...
  Object.keys(self._map.pkg).map(function(pkgId){
    var currentPkg=self._map.pkg[pkgId];
    if(currentPkg.asyncs){
      var msg = {
        target: currentPkg.uri,
        file: self._host,
        ret: currentPkg.uri
      };
      ...
      var entryId=self._map.pkg[pkgId].entry;
      var node = self.getNode(entryId);
      var file = self.getFileById(entryId);
      var moduleId = node.extras && node.extras.moduleId || file && file.moduleId || id.replace(/\.js$/i, '');
      res[moduleId]={
        url:entryId,
        type:file.type,
